...
## 13. Tổ chức lại cấu trúc Blog, di chuyển mã nguồn mẫu, đồng bộ giao diện & Prettier (2026-06-27)

- **Tổ chức lại cấu trúc thư mục Blog**: Di chuyển toàn bộ các bài viết HTML từ thư mục gốc `blog/` vào các thư mục con tương ứng của từng series: `blog/c/`, `blog/cpp/`, `blog/js/`, `blog/canvas/`, và `blog/bash/`. Các bài viết độc lập (hướng dẫn công cụ) vẫn giữ nguyên nằm ở thư mục gốc `blog/`.
- **Xóa bỏ thư mục codes/ & Đồng bộ co-location**: Di chuyển các file mã nguồn mẫu/file mô phỏng tương ứng từ `blog/codes/` vào trực tiếp thư mục bài học của từng lộ trình học, sau đó xóa bỏ hoàn toàn thư mục `blog/codes/`.
- **Đơn giản hóa đường dẫn**: Cập nhật lại toàn bộ các liên kết tương đối (CSS, JS, hình ảnh), đường dẫn tải file code mẫu, liên kết chéo giữa các bài học, sitemap (`sitemap.xml`) cũng như chỉ mục tìm kiếm (`blog/search-index.json`).
- **Thống nhất giao diện lộ trình**: Đồng bộ hóa toàn bộ giao diện danh sách bài học của các lộ trình C++, JS, Canvas, Bash giống hệt lộ trình C (thêm mũi tên điều hướng, chỉnh chiều cao hợp lý và tối ưu hiển thị responsive trên thiết bị di động).
- **Cải tiến điều hướng bài viết trước/sau (Prev/Next)**: Cập nhật CSS của `.article-related` và loại bỏ các mũi tên thủ công trong HTML để điều hướng hiển thị chuẩn xác (mũi tên `←` bên trái tiêu đề cho bài trước, và mũi tên `→` bên phải tiêu đề cho bài sau).
- **Định dạng code tự động**: Cấu hình Prettier (`.prettierrc`, `.vscode/settings.json`, `package.json`), sửa đổi các lỗi cú pháp HTML/JS cũ (lỗi lồng dấu nháy kép ở các nút trắc nghiệm, unescaped ampersands và dấu so sánh `<`/`>` trong code blocks) và chạy định dạng làm sạch toàn bộ dự án.
