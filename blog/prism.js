...
/* TypeScript — them cho loat bai NestJS (/blog/build/). Ban chinh thuc cua
   Prism, mo rong tu grammar javascript da co san trong file nay. */
(function (Prism) {
  Prism.languages.typescript = Prism.languages.extend('javascript', {
    'class-name': {
      pattern:
        /(\b(?:class|extends|implements|instanceof|interface|new|type)\s+)(?!keyof\b)(?!keyof[\s\S])[_$a-zA-Z\xA0-￿](?:(?!\s)[$\w\xA0-￿])*(?:\s*<(?:[^<>]|<(?:[^<>]|<[^<>]*>)*>)*>)?/,
      lookbehind: true,
      greedy: true,
      inside: null,
    },
