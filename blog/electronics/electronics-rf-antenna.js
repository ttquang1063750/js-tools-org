document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateSVG() {
    ...
    if (mode === 'tuning') {
      ...
      // Loop coils
      html += '<path d="M 110 50 Q 115 40, 120 50 Q 125 40, 130 50 Q 135 40, 140 50 Q 145 40, 150 50" fill="none" stroke="#89b4fa" stroke-width="2" />';
      ...
      const isTuned = Math.abs(L * C - 159155*159155 / (fStation*fStation)) < 3000 || valTuneStatus.textContent.includes('cộng hưởng');
      ...
      html += '<text x="130" y="160" fill="#a6e3a1" font-size="11" text-anchor="middle">LC Resonator (Chọn sóng)</text>';
    } 
    else {
      ...
      if (qual.includes('Trong trẻo')) {
        ...
        html += '<path d="M 305 70 L 310 75 L 305 80 L 310 85 L 305 90" fill="none" stroke="#f9e2af" stroke-width="1" />';
      }
      
      html += '<text x="185" y="155" fill="#a6e3a1" font-size="11" text-anchor="middle">AM Detector & Filter (Tách sóng & Lọc)</text>';
    }
  ...
  function drawScope() {
    ...
    if (mode === 'tuning') {
      ...
      ctxScope.beginPath();
      ctxScope.moveTo(10, h - 10); ctxScope.lineTo(w - 10, h - 10); // horizontal axis
      ctxScope.moveTo(10, 10); ctxScope.lineTo(10, h - 10); // vertical axis
      ctxScope.stroke();
