...
| Series 11                        | Thiết Kế Vi Mạch Số & FPGA (VLSI)       | 12/14          | 14       | 86%         |
...
- ⚠️ **Hạng mục xây nặng nhất series** (độ khó ~ sql.js workbench) — phải làm sớm ngay sau hub vì mọi bài SystemVerilog đều nhúng lại nó ở cấu hình nhỏ hơn.
...
- **Khối "📚 Điều kiện tiên quyết" BẮT BUỘC** ngay sau đoạn mở đầu mỗi bài (`.callout--note`): liệt kê link nội bộ (bài Series 10/11 liên quan, Series C/C++) + link ngoài (HDLBits, ChipVerify, EDA Playground, tài liệu Yosys/Verilator) theo ghi chú _Tiên quyết_ ở Phần III. Đây là đặc điểm nhận diện của series, không được bỏ.
...
- **Bài 11 — SystemC & Mô hình hoá mức hệ thống (TLM):** 11.1 Khoảng trống giữa ý tưởng kiến trúc và RTL: cần mô hình chạy nhanh gấp trăm–nghìn lần để khám phá thiết kế và cho team software viết driver sớm (shift-left) · 11.2 SystemC = thư viện C++: `SC_MODULE`, `SC_METHOD`/`SC_THREAD`, `sc_signal`, delta-cycle — đối chiếu 1-1 với chính engine VeriLite của series (scheduler sự kiện) · 11.3 TLM-2.0: giao dịch (transaction) thay cho tín hiệu từng bit, loosely-timed vs approximately-timed, virtual platform trong công nghiệp · 11.4 Bảng so sánh các mức mô hình: untimed → TLM-LT → TLM-AT → RTL → gate-level (tốc độ mô phỏng ↔ độ chính xác chu kỳ) — khi nào dùng mức nào · 11.5 Thực hành: cùng hệ producer–consumer chạy hai tầng TLM (JS mô phỏng API SystemC) và RTL, đếm số sự kiện mô phỏng chênh lệch hàng trăm lần. _Tiên quyết: Series C/C++ (class, template cơ bản); bài 7 series này (FIFO)._
