...
| Series 15                        | Kiến Trúc Máy Tính: Từ Logic Đến Lượng Tử | 11/12          | 12       | 92%         |
...
> thật đối chiếu demo trực quan. Nội dung 100% tiếng Việt. Trạng thái: **11/12 bài** (Bài 11 xong 2026-07-23).
>
...
> Tiên quyết ngoài: Series 10 Điện Tử (cổng logic), Series C (con trỏ/bộ nhớ — nhẹ).
>
> **CPUJS — engine dùng chung** (`blog/cpu/cpu-core.js`, ES module, self-test `node cpu-core.js` với số
> verified — ĐÚNG kỷ luật "verify trước khi viết prose" của DSP/VLSI/AI): xây DẦN qua từng bài, mỗi bài
> chỉ mở rộng đúng phần mình dạy (xem "cpu-core.js build-out" mỗi bài). **Bắt buộc:** mọi con số trong
> prose/quiz (kết quả ALU, cờ, độ trễ pipeline có stall, CPI hiệu dụng, AMAT, dung lượng page table,
> IPC Tomasulo, yield...) phải được engine sinh ra & self-test khoá lại TRƯỚC, không bịa tay — đây chính
> là lớp lưới đã bắt hàng loạt bug tính toán ẩn ở VLSI (xem `check-lesson.md` PHẦN D #10, #14–17). Các
> bài nặng số (4, 6, 7, 11) rủi ro cao nhất nếu bỏ bước này.
>
> **Core visualizer** (dựng nặng nhất, tái dùng nhiều bài): Datapath/Pipeline Simulator RISC-V
> (`cpu-pipeline-sim.html`) — dùng lại ở Bài 3 (datapath đơn chu kỳ) → 4 (pipeline + forwarding) → 5
> (dự đoán nhánh) → 6 (OOO); Cache L1 Simulator tái dùng ở Bài 7–8. Các bài còn lại có demo lẻ nhẹ, tất
> cả cùng import `cpu-core.js`. KaTeX nạp hầu hết các bài; giữ quy tắc `\text{}` chỉ ASCII (bug diacritics).
>
> **Thứ tự bài (đã sắp lại 2026-07-13):** gom trọn mạch "làm MỘT nhân chạy nhanh hơn" (ILP) liền nhau —
> pipeline → hazard → dự đoán nhánh → **OOO/Tomasulo** (Bài 6, kéo lên từ vị trí 8 cũ vì nó xây thẳng
> trên mô hình pipeline Bài 4–5, KHÔNG cần cache/VM) — rồi mới sang phân cấp bộ nhớ (cache → bộ nhớ ảo,
> Bài 7–8), sau đó kiến trúc hiện đại (Bài 9–11). Bài 12 (Lượng tử) là **chân trời mở rộng, KHÔNG phải
> capstone tổng hợp**: nó là một nhánh kiến trúc riêng, không tái dùng kiến thức Bài 1–11 — phải nói rõ
> điều này trong bài để người đọc không chờ một sự tổng kết không xảy ra.

**Chặng 1 — Nền tảng: từ transistor đến tập lệnh (Bài 1–3)**

- **Bài 1 — Cổng Logic đến Đơn Vị ALU** (`cpu-logic-alu.html`): 1.1 Đại số Boolean & Cổng logic cơ bản: Cổng AND, OR, NOT, XOR ở mức bit (why: Đây là các gạch xây dựng cơ bản của mọi cổng tính toán số học; when: Cổng XOR dùng cho phép cộng và so sánh, cổng AND/OR dùng cho mặt nạ bit; pitfall: Nhầm lẫn giữa phép toán logic bitwise `&` và logic Boolean `&&` trong thiết kế phần cứng) · 1.2 Mạch cộng bán phần & toàn phần (Half/Full Adder): Cấu trúc mạch cộng, bit nhớ Carry-out (why: CPU cần cộng các số nhị phân ở tốc độ cao; when: Full Adder khi cần cộng bit có nhớ từ hàng trước; pitfall: Bỏ qua trễ lan truyền của bit nhớ Carry Propagation Delay trong mạch cộng Ripple Carry) · 1.3 Thiết kế khối ALU 4-bit và cờ trạng thái: Cấu trúc ALU ghép nối bộ cộng và chọn phép toán, cờ Zero, Sign, Overflow (why: Giúp CPU đưa ra quyết định rẽ nhánh dựa trên trạng thái tính toán; when: Luôn hoạt động trong giai đoạn EX - Execute; pitfall: Hiểu sai cơ chế tràn số Overflow trên số có dấu signed vs không dấu unsigned) · 1.4 Thực hành: Bộ mô phỏng ALU 4-bit tương tác — chọn toán tử nhị phân và quan sát trạng thái các cờ. **cpu-core.js build-out:** half/full adder, bộ cộng ripple-carry, ALU 4-bit (ADD/SUB/AND/OR/XOR) + cờ Z/S/C/V (ĐÃ XONG, 41 self-test checks). _Tiên quyết: Series 10 Điện Tử (cổng logic)._
- **Bài 2 — Kiến Trúc Von Neumann & Tập Lệnh ISA** (`cpu-von-neumann-isa.html`): 2.1 Mô hình Von Neumann vs Harvard: Kiến trúc bus chung vs bus riêng cho lệnh và dữ liệu (why: Tránh nút cổ chai Von Neumann bottleneck; when: Von Neumann ở hệ thống bộ nhớ chính, Harvard ở cache L1; pitfall: Lưu đè dữ liệu lên vùng nhớ mã lệnh Self-modifying code gây lỗi bảo mật hoặc hành vi không xác định) · 2.2 Chu kỳ Fetch-Decode-Execute: Các bước nạp lệnh từ RAM, giải mã lệnh và thực thi, thanh ghi PC và IR (why: Đây là vòng lặp vô hạn điều khiển toàn bộ hoạt động của CPU; when: Chạy liên tục từ lúc khởi động chip; pitfall: PC tăng trước khi giải mã xong lệnh nhảy gây sai lệch địa chỉ đích) · 2.3 Kiến trúc tập lệnh ISA: Phân biệt RISC vs CISC, ARM vs x86 (why: Thiết kế ISA ảnh hưởng trực tiếp đến độ phức tạp của phần cứng giải mã; when: CISC cho tương thích ngược cao, RISC cho thiết kế hiệu quả điện năng; pitfall: "RISC luôn nhanh hơn CISC" — thực tế CISC hiện đại dịch các chỉ lệnh phức tạp thành các micro-ops chạy trên nhân RISC ngầm) · 2.4 Thực hành: Bộ giả lập Toy CPU đơn giản — xem RAM và thanh ghi cập nhật qua từng bước Fetch/Decode/Execute. **cpu-core.js build-out:** Toy CPU fetch-decode-execute trên tập lệnh mini + thanh ghi PC/IR (self-test: chạy 1 chương trình nhỏ ra đúng trạng thái RAM/thanh ghi). _Tiên quyết: Bài 1._
- **Bài 3 — Hợp Ngữ RISC-V & Đường Đi Của Dữ Liệu (Datapath)** (`cpu-riscv-datapath.html`): 3.1 Tập lệnh RISC-V (RV32I): Cấu trúc lệnh R-type, I-type, S-type (why: RISC-V có thiết kế cực kỳ sạch, không có overhead legacy, dễ học nhất; when: Khi viết code hợp ngữ cấp thấp hoặc thiết kế chip nhúng mới; pitfall: Ghi đè vào thanh ghi cứng x0 vốn luôn bằng 0) · 3.2 Dịch Assembly sang Mã máy: Quá trình ánh xạ chỉ chỉ thị văn bản sang chuỗi 32-bit nhị phân, opcodes, registers encoding (why: CPU chỉ hiểu tín hiệu điện nhị phân; when: Linker và Assembler thực hiện tự động; pitfall: Nhầm lẫn giữa địa chỉ tương đối PC-relative offset và địa chỉ tuyệt đối trong lệnh nhảy) · 3.3 Thiết kế Datapath đơn chu kỳ (Single-Cycle): Đường đi của dữ liệu qua các khối ALU, Register File, RAM, giới hạn chu kỳ xung nhịp bởi lệnh dài nhất LW (why: Đây là mô hình CPU trực quan nhất nhưng hiệu năng kém nhất; when: Chỉ dùng để giảng dạy hoặc trong các vi điều khiển tần số siêu thấp; pitfall: Giả định rằng mọi lệnh trong Single-Cycle CPU đều tốn thời gian thực thi bằng nhau trên silicon thật) · 3.4 Thực hành: Trực quan hóa Datapath đơn chu kỳ RISC-V — gõ lệnh Assembly và nhìn đường dẫn phát sáng trên sơ đồ khối CPU. **cpu-core.js build-out:** RV32I assembler/decoder (R/I/S-type ↔ 32-bit) + datapath đơn chu kỳ (self-test: assemble→decode khứ hồi + kết quả từng lệnh). _Tiên quyết: Bài 2._

**Chặng 2 — Song song cấp lệnh (ILP): làm MỘT nhân chạy nhanh hơn (Bài 4–6)**

- **Bài 4 — Pipeline CPU & Xung Đột Dữ Liệu (Data Hazards)** (`cpu-pipeline-hazards.html`): 4.1 Cơ chế Pipeline 5 giai đoạn: Chia nhỏ công việc để thực thi song song, các thanh ghi đệm liên giai đoạn (why: Tối ưu hóa thông lượng throughput của CPU thay vì giảm độ trễ latency của một lệnh riêng lẻ; when: Là thiết kế mặc định trên hầu hết vi xử lý hiệu năng cao; pitfall: Lầm tưởng pipeline giúp rút ngắn thời gian thực thi của một chỉ lệnh đơn lẻ) · 4.2 Tính toán hiệu năng Pipeline: Công thức tính thời gian chạy $T = (\text{Instructions} + \text{Stages} - 1 + \text{Stall Cycles}) \times t_{clk}$, ví dụ thực tế tính toán thời gian chạy 1 triệu chỉ lệnh trên CPU 5 stage ở xung nhịp 2GHz khi có và không có stall (why: Giúp kỹ sư định lượng chính xác hiệu năng lý thuyết vs thực tế silicon; when: Khi đánh giá hiệu quả thiết kế và viết phần mềm tối ưu; pitfall: Quên tính đến chu kỳ nạp ban đầu pipeline startup latency khi số lượng lệnh nhỏ) · 4.3 Xung đột dữ liệu (Data Hazards): Read-After-Write (RAW), Write-After-Read (WAR), Write-After-Write (WAW), cơ chế giải quyết bằng bộ chuyển tiếp dữ liệu (Forwarding) và chèn bong bóng (Stall/NOP) (why: Đảm bảo tính đúng đắn khi lệnh sau phụ thuộc dữ liệu lệnh trước; when: Xảy ra liên tục trong luồng thực thi; pitfall: Kỹ thuật Forwarding không thể giải quyết được xung đột Load-use do lệnh LW gây ra, vẫn bắt buộc phải Stall 1 chu kỳ) · 4.4 Phân tích ví dụ thực tế & tính toán Stall: Bài tập tính toán số chu kỳ clock bị mất cho một đoạn hợp ngữ cụ thể khi tắt/bật Forwarding, vẽ giản đồ thời gian (pipeline clock grid) chi tiết · 4.5 Thực hành: Mô phỏng Pipeline 5 giai đoạn RISC-V tương tác — chạy từng tick clock xem luồng lệnh chồng lấp và cơ chế Forwarding xử lý hazard. **cpu-core.js build-out:** mô hình pipeline 5 stage, phát hiện hazard RAW/load-use, forwarding, hàm tính $T$ và CPI có stall (self-test đối chiếu công thức + giản đồ tay). _Tiên quyết: Bài 3._
- **Bài 5 — Dự Đoán Nhánh & Lỗi Bảo Mật Spectre** (`cpu-branch-prediction.html`): 5.1 Xung đột điều khiển (Control Hazards) & Bộ dự đoán nhánh: Ảnh hưởng của lệnh rẽ nhánh điều kiện (BEQ, BNE) lên luồng nạp lệnh (why: CPU bị trễ 1-3 chu kỳ để biết địa chỉ nạp lệnh tiếp theo; when: Xảy ra tại mọi cấu trúc rẽ nhánh if/else, vòng lặp; pitfall: Cho rằng tối ưu hóa số dòng code nguồn sẽ loại bỏ xung đột rẽ nhánh trong mã máy) · 5.2 Bộ dự đoán nhánh tĩnh & động (1-bit, 2-bit Predictor): Cơ chế dự đoán tĩnh (NT/T) vs dự đoán động sử dụng máy trạng thái 2-bit và bảng lịch sử nhánh BHT (why: Tăng độ chính xác dự đoán từ ~50% lên trên 90% dựa trên tính lặp lại; when: Tích hợp trong mọi CPU hiện đại để giữ đầy đường ống pipeline; pitfall: Bộ dự đoán 1-bit dễ bị dao động liên tục ở các vòng lặp lồng nhau, gây ra 2 lần đoán sai liên tiếp) · 5.3 Tính toán Jitter và Penalty chu kỳ: Công thức tính CPI hiệu dụng $CPI_{eff} = CPI_{ideal} + \text{Branch Frequency} \times \text{Misprediction Rate} \times \text{Penalty Cycles}$, bài toán thực tế tính CPI hiệu dụng cho chương trình có 20% lệnh rẽ nhánh và tỷ lệ đoán sai 10% (why: Giúp đánh giá chính xác thiệt hại hiệu năng do đoán sai nhánh; when: Khi tối ưu hóa thuật toán và thiết kế bộ dự đoán; pitfall: Đánh giá thấp ảnh hưởng của các câu lệnh rẽ nhánh không thể dự đoán unpredictable branches) · 5.4 Thực thi suy đoán (Speculative Execution) & Lỗ hổng Spectre: CPU chạy trước các lệnh trong nhánh được dự đoán, cơ chế khôi phục kiến trúc và dấu vết rò rỉ trong Cache, cách khai thác Spectre qua kênh kề Side-channel attack (why: Hiểu lỗ hổng bảo mật phần cứng nghiêm trọng nhất lịch sử CPU để thiết kế các cơ chế phòng ngự; when: Khi phát triển OS/Compiler hoặc phần cứng an toàn; pitfall: Cho rằng lỗi bảo mật phần cứng Spectre chỉ có thể khắc phục hoàn toàn bằng phần mềm mà không tốn hiệu năng) · 5.5 Thực hành: Bộ mô phỏng dự đoán nhánh 2-bit — đo tỷ lệ dự đoán chính xác thực tế khi chạy các cấu trúc vòng lặp và rẽ nhánh khác nhau. **cpu-core.js build-out:** bộ dự đoán nhánh 1-bit & 2-bit (FSM + BHT), hàm tính CPI hiệu dụng (self-test trên chuỗi rẽ nhánh biết trước tỷ lệ đoán sai). _Tiên quyết: Bài 4._
- **Bài 6 — Song Song Cấp Lệnh & Out-of-Order Execution** (`cpu-ooo-execution.html`): 6.1 Song song cấp lệnh (ILP) & CPU Superscalar: Khái niệm phát và thực thi nhiều lệnh cùng một chu kỳ (Issue Width), bộ xử lý vô hướng Scalar vs siêu vô hướng Superscalar (why: Vượt qua giới hạn hiệu năng của CPU đơn luồng tuần tự; when: Lõi CPU hiệu năng cao; pitfall: Hiệu năng thực tế bị nghẽn bởi các phụ thuộc dữ liệu thật RAW) · 6.2 Phụ thuộc dữ liệu giả & Kỹ thuật đổi tên thanh ghi (Register Renaming): Phân tích xung đột dữ liệu giả WAR và WAW (why: Loại bỏ các phụ thuộc giả tạo ra do số lượng thanh ghi kiến trúc bị giới hạn; when: Khi chuẩn bị nạp lệnh vào hàng đợi thực thi không tuần tự; pitfall: Nhầm lẫn giữa phụ thuộc dữ liệu thật RAW - không thể triệt tiêu và phụ thuộc dữ liệu giả - có thể triệt tiêu bằng đổi tên thanh ghi) · 6.3 Thuật toán Tomasulo & Hàng đợi Out-of-Order: Chi tiết hoạt động của trạm đặt chỗ (Reservation Stations), Bus dữ liệu chung CDB và thanh ghi đệm kết quả ROB để thực thi không theo thứ tự nhưng cam kết kết quả theo thứ tự (In-Order Commit) (why: Tối ưu hóa việc sử dụng các đơn vị chức năng ALU/FPU khi có lệnh bị kẹt do trễ bộ nhớ; when: Lõi CPU hiện đại (x86, ARM); pitfall: Lệnh bị nghẽn ở ROB khiến hàng ROB bị đầy và gây dừng CPU - ROB Stall) · 6.4 Tính toán IPC thực tế và hiệu quả thuật toán: Bài toán thực tế cho một đoạn hợp ngữ, thực hiện mô phỏng đổi tên thanh ghi và vẽ bảng lập lịch thực thi Tomasulo để tính tổng số chu kỳ và IPC thực tế (why: Đo lường định lượng chính xác hiệu năng song song cấp lệnh thực tế; when: Khi đánh giá vi kiến trúc vi xử lý; pitfall: Đánh đổi hiệu năng lấy chi phí diện tích silicon và mức tiêu thụ năng lượng khổng lồ của mạch logic điều khiển OOO cực kỳ phức tạp) · 6.5 Thực hành: Bộ giả lập Tomasulo thu nhỏ tương tác — gõ chuỗi lệnh Assembly và quan sát sự thay đổi trạng thái của các Reservation Stations and Reorder Buffer ROB. **cpu-core.js build-out:** đổi tên thanh ghi (register renaming) + lập lịch Tomasulo (reservation station, ROB, in-order commit), tính IPC (self-test trên đoạn hợp ngữ có phụ thuộc giả WAR/WAW). _Tiên quyết: Bài 4–5 (pipeline & hazard — OOO xây thẳng trên đây, KHÔNG cần cache/VM)._

**Chặng 3 — Phân cấp bộ nhớ: nuôi con quái vật đói dữ liệu (Bài 7–8)**

- **Bài 7 — Phân Cấp Bộ Nhớ & Kiến Trúc Cache** (`cpu-cache-memory.html`): 7.1 Nút thắt cổ chai bộ nhớ (Memory Wall) & Nguyên lý Locality: Khoảng cách tốc độ CPU vs DRAM, nguyên lý Temporal Locality và Spatial Locality (why: Sử dụng bộ nhớ đệm SRAM siêu nhanh để che giấu độ trễ khổng lồ của DRAM; when: Áp dụng trong mọi hệ thống máy tính hiện đại; pitfall: Duyệt mảng 2D theo cột thay vì theo dòng làm mất tính Spatial Locality, khiến tốc độ chạy giảm hàng chục lần) · 7.2 Cấu trúc ánh xạ Cache (Direct-Mapped, Set-Associative): Cấu trúc định địa chỉ Index, Tag, Offset, cách ánh xạ bộ nhớ vào Cache (why: Giúp CPU tìm kiếm dữ liệu cực nhanh và giảm thiểu Conflict Miss; when: Khi thiết kế L1/L2 Cache; pitfall: Thiết kế Set-Associative tăng độ phức tạp phần cứng và tiêu tốn năng lượng hơn Direct-Mapped) · 7.3 Tính toán chỉ số hiệu năng Cache: Công thức tính thời gian truy cập trung bình $AMAT = T_{\text{Hit}} + \text{Miss Rate} \times T_{\text{Miss\_Penalty}}$, bài toán thực tế tính AMAT cho hệ thống có L1 và L2 Cache để so sánh với hệ thống chỉ có L1 (why: Cơ sở để kỹ sư chip quyết định dung lượng và cấu hình phân cấp Cache tối ưu; when: Khi phân tích hiệu năng hệ thống nhớ; pitfall: Nhầm lẫn giữa tỷ lệ Miss Rate cục bộ Local vs toàn cục Global trong hệ thống nhiều cấp Cache) · 7.4 Thực hành: Cache L1 Simulator — nhập các thao tác đọc/ghi địa chỉ, tính toán nhẩm Tag/Index/Offset và quan sát bảng dòng Cache cập nhật trạng thái Hit/Miss trực quan. **cpu-core.js build-out:** tách địa chỉ tag/index/offset, cache direct-mapped & set-associative (LRU), tính AMAT nhiều cấp (self-test chuỗi truy cập biết trước Hit/Miss). _Tiên quyết: Bài 3 (truy cập bộ nhớ trong datapath)._
- **Bài 8 — Bộ Nhớ Ảo & Khối TLB** (`cpu-virtual-memory-tlb.html`): 8.1 Bộ nhớ ảo (Virtual Memory) & Phân trang (Paging): Khái niệm địa chỉ ảo vs địa chỉ vật lý, cấu trúc Page Table, trang bộ nhớ mặc định 4KB (why: Cho phép chạy chương trình lớn hơn dung lượng RAM vật lý và cô lập an toàn không gian nhớ giữa các tiến trình; when: Yêu cầu bắt buộc đối với mọi hệ điều hành đa nhiệm hiện đại; pitfall: Hiện tượng quá tải trang Thrashing xảy ra liên tục khi thiếu RAM vật lý, khiến CPU dành toàn bộ thời gian để tráo trang nạp từ đĩa cứng và làm đơ hệ thống) · 8.2 Tính toán cấu trúc và dung lượng Page Table: Bài toán tính số bit biểu diễn địa chỉ ảo, số trang ảo, và tính toán dung lượng RAM cần thiết để lưu Page Table đơn cấp vs Page Table đa cấp cho kiến trúc 32-bit và 64-bit (why: Định lượng được overhead dung lượng lưu trữ của bảng trang để tối ưu hóa thiết kế; when: Khi cấu hình MMU trong lõi vi xử lý; pitfall: Bảng trang đơn cấp tốn quá nhiều RAM không cần thiết để lưu các trang chưa từng sử dụng) · 8.3 Khối tăng tốc dịch địa chỉ TLB (Translation Lookaside Buffer): Cơ chế hoạt động của TLB làm Cache cho Page Table, các trạng thái TLB Hit vs TLB Miss (why: Tránh việc phải truy cập DRAM 2 lần chỉ để đọc bảng trang, giúp tăng tốc độ dịch địa chỉ ảo thành vật lý; when: MMU tự động thực hiện trên mỗi lệnh truy cập bộ nhớ; pitfall: Hiện tượng TLB Flush xảy ra mỗi khi chuyển đổi ngữ cảnh tiến trình Context Switch làm giảm hiệu năng hệ thống ngay sau đó) · 8.4 Thực hành: Sơ đồ dịch địa chỉ ảo động và mô phỏng TLB Hit/Miss — nhập địa chỉ ảo, tính toán Virtual Page Number và Offset, theo dõi MMU tra cứu TLB và Page Table để xuất ra Physical Address vật lý. **cpu-core.js build-out:** dịch địa chỉ ảo→vật lý qua page table + TLB, tính dung lượng bảng trang đơn/đa cấp (self-test khứ hồi VPN/offset + số liệu 32/64-bit). _Tiên quyết: Bài 7 (cache — TLB chính là cache cho bảng trang)._

**Chặng 4 — Kiến trúc hiện đại & chân trời tương lai (Bài 9–12)**

- **Bài 9 — Apple Silicon & Kiến Trúc Bộ Nhớ Thống Nhất (UMA)** (`cpu-apple-silicon-uma.html`): 9.1 Hệ thống trên chip (SoC) vs Bo mạch truyền thống: Xu hướng tích hợp CPU, GPU, RAM, NPU trên cùng một đế silicon (why: Giảm cực đại khoảng cách truyền dẫn, tiết kiệm năng lượng và giảm độ trễ; when: Chuẩn thiết kế cho thiết bị di động và máy tính hiện đại; pitfall: Không thể nâng cấp RAM hoặc thay thế các linh kiện rời rạc khi hỏng hóc) · 9.2 Nhân lớn/nhỏ (big.LITTLE): Thiết kế phân chia nhân Firestorm (Hiệu năng) vs Icestorm (Tiết kiệm điện) trên Apple M1 và vai trò của bộ lập lịch OS Thread Scheduler (why: Cân bằng tối ưu giữa việc xử lý tác vụ nặng và duy trì thời lượng pin lâu; when: Tác vụ thay đổi từ xử lý nền sang ứng dụng nặng; pitfall: Lập lịch sai lầm đẩy tác vụ nền lên nhân hiệu năng cao gây hao pin vô ích) · 9.3 Kiến trúc bộ nhớ thống nhất (UMA - Unified Memory Architecture): Chia sẻ chung một bể RAM LPDDR5 băng thông cực lớn giữa CPU và GPU, loại bỏ việc sao chép (why: Triệt tiêu hoàn toàn nút thắt cổ chai truyền tải dữ liệu qua bus PCIe chậm chạp; when: Xử lý đồ họa nặng, dựng video 8K hoặc chạy mô hình LLM cục bộ; pitfall: RAM UMA đắt đỏ và không thể nâng cấp dung lượng) · 9.4 Bài toán tính toán băng thông và tiết kiệm năng lượng: Bài toán tính dung lượng dữ liệu truyền tải và thời gian trễ khi render một khung hình 4K (3840x2160, 32-bit màu) khi truyền qua bus PCIe Gen 4 x16 (sao chép CPU sang GPU) so với truy cập trực tiếp trên Apple Silicon UMA (why: Chứng minh bằng con số cụ thể sự tiết kiệm năng lượng và băng thông của UMA; when: Phân tích so sánh hiệu năng vi kiến trúc; pitfall: So sánh thô xung nhịp CPU mà bỏ qua tốc độ băng thông bộ nhớ thực tế) · 9.5 Thực hành: Sơ đồ tương tác so sánh hai luồng truyền dữ liệu — kéo một tác vụ render nặng và theo dõi số GB/s truyền qua bus PCIe (PC truyền thống) vs UMA (Apple Silicon) cùng thời gian hoàn thành ước tính. **cpu-core.js build-out:** mô hình băng thông (bytes/khung hình, thời gian truyền PCIe vs truy cập trực tiếp UMA) — self-test số liệu khung 4K. _Tiên quyết: Bài 7–8 (phân cấp bộ nhớ & bộ nhớ ảo)._
- **Bài 10 — Tăng Tốc Phần Cứng: GPU, NPU & AMX** (`cpu-hardware-acceleration.html`): 10.1 Kiến trúc SIMD vs SIMT: Khái niệm Single Instruction Multiple Data vs Single Instruction Multiple Threads, cách thức hoạt động của Shader Cores trong GPU (why: Tối ưu hóa xử lý song song mức độ lớn cho đồ họa và deep learning; when: Khi thực thi các toán tử xử lý ma trận và mảng lớn; pitfall: Lầm tưởng GPU có thể thay thế CPU chạy mọi chương trình, thực tế GPU rất yếu đối với các đoạn code rẽ nhánh tuần tự phức tạp) · 10.2 Kiến trúc NPU & Công cụ tăng tốc AI: Nguyên lý hoạt động của Tensor Core và Neural Processing Unit, cơ chế phần cứng tăng tốc phép nhân ma trận (why: Tăng tốc độ chạy suy luận/huấn luyện mạng thần kinh nhân tạo gấp hàng chục lần so với CPU thông thường; when: Khi xử lý mô hình deep learning cục bộ; pitfall: Lượng tử hóa trọng số Quantization từ FP32 xuống INT8 không chuẩn xác làm sụt giảm nghiêm trọng chất lượng mô hình AI) · 10.3 Apple AMX (Apple Matrix Coprocessor): Khối đồng xử lý ma trận tích hợp ẩn trong lõi Apple CPU, tập lệnh độc quyền và cách tối ưu hóa (why: Thực thi nhanh chóng phép toán ma trận lớn mà không cần chuyển dữ liệu sang GPU; when: Khi chạy CoreML hoặc Accelerate framework; pitfall: AMX là kiến trúc đóng hoàn toàn, không có tài liệu lập trình công khai mà phải dùng qua các thư viện hệ thống) · 10.4 Tính toán thông lượng FLOPs nhân ma trận: Bài toán tính số phép tính dấu phẩy động cần thiết để nhân hai ma trận $N \times N$, so sánh hiệu năng thực tế (TFLOPS) giữa CPU chạy $O(N^3)$, CPU SIMD vector hóa và GPU/AMX song song (why: Định lượng và chứng minh trực quan ưu thế của bộ tăng tốc chuyên dụng; when: Khi tối ưu hóa thư viện deep learning; pitfall: Bỏ qua overhead nạp dữ liệu vào thanh ghi vector khi kích thước ma trận quá nhỏ) · 10.5 Thực hành: Mô phỏng nhân ma trận tương tác — so sánh trực quan tốc độ và số lượng phép tính diễn ra đồng thời giữa CPU tuần tự vs SIMD vs GPU/AMX song song. **cpu-core.js build-out:** đếm FLOPs nhân ma trận $N\times N$, so tuần tự $O(N^3)$ vs SIMD vs song song (self-test số phép tính khớp công thức). _Tiên quyết: Bài 9._
- **Bài 11 — Điểm Cuối Định Luật Moore & Đóng Gói Chiplet** (`cpu-moore-law-chiplets.html`): 11.1 Giới hạn vật lý của Silicon & Sự kết thúc của Định luật Moore: Hiện tượng hiệu ứng đường hầm lượng tử khi cổng gate dưới 3nm, giới hạn mật độ nhiệt Dark Silicon (why: Hiểu lý do tại sao không thể tăng xung nhịp CPU lên cao hơn nữa và tốc độ thu nhỏ transistor đang chạm giới hạn; when: Khi nghiên cứu sự dịch chuyển công nghệ vi xử lý; pitfall: Cho rằng tốc độ vi xử lý sẽ ngừng tăng, thực tế hiệu năng vẫn tăng nhờ tối ưu vi kiến trúc) · 11.2 Xu hướng thiết kế Chiplet: Chia nhỏ lõi CPU lớn thành nhiều die silicon nhỏ hơn đảm nhận các vai trò khác nhau (CPU, I/O, Cache) đặt trên interposer (why: Tăng tỷ lệ sản xuất thành công Yield Rate và giảm giá thành sản xuất; when: Áp dụng trên AMD Ryzen, Intel Meteor Lake, Apple M-Ultra; pitfall: Overhead độ trễ giao tiếp inter-chiplet cao hơn thiết kế nguyên khối monolithic) · 11.3 Công nghệ đóng gói 3D IC & Kết nối TSV: Xếp chồng silicon theo chiều dọc và kết nối qua lỗ xuyên silicon TSV (why: Tăng dung lượng và băng thông giao tiếp lên cực đại trong diện tích bo mạch nhỏ; when: 3D V-Cache của AMD, bộ nhớ HBM xếp chồng sát GPU; pitfall: Gặp thách thức cực lớn trong việc giải nhiệt cho các tầng silicon dưới cùng) · 11.4 Bài toán tính toán Yield Rate và Chi phí sản xuất: Sử dụng mô hình Poisson/Murphy để tính toán tỷ lệ chip monolithic đạt chuẩn và so sánh chi phí sản xuất monolithic die lớn vs thiết kế chiplet ghép nhiều die nhỏ (why: Chứng minh động lực kinh tế mạnh mẽ thúc đẩy các nhà sản xuất chip chuyển sang chiplet; when: Khi lập kế hoạch sản xuất phần cứng; pitfall: Bỏ qua chi phí đóng gói packaging và kiểm thử testing phức tạp của chiplet trong bài toán tổng thể) · 11.5 Thực hành: Trình giả lập tính toán Yield & Chi phí Wafer — tự nhập kích thước die, số lượng chiplet và mật độ lỗi defect density để xem tỷ lệ đạt và chi phí sản xuất trực quan. **cpu-core.js build-out:** mô hình yield Poisson/Murphy + chi phí wafer, so monolithic die lớn vs chiplet nhiều die nhỏ (self-test đối chiếu công thức). _Tiên quyết: Bài 10._
- **Bài 12 — Chân trời: Kiến Trúc Máy Tính Lượng Tử (Quantum Computing)** _(outlook mở rộng, KHÔNG phải capstone tổng hợp — nhánh riêng, không nối tiếp Bài 1–11)_ (`cpu-quantum-computing.html`): 12.1 Bit cổ điển vs Qubit lượng tử: Trạng thái nhị phân 0/1 vs trạng thái chồng chập superposition biểu diễn bằng quả cầu Bloch và vector trạng thái $|\psi\rangle = \alpha|0\rangle + \beta|1\rangle$ với $|\alpha|^2 + |\beta|^2 = 1$ (why: Cho phép xử lý và biểu diễn không gian thông tin song song quy mô siêu khủng; when: Giải quyết các bài toán tối ưu hóa phức tạp; pitfall: Lầm tưởng máy tính lượng tử có thể tăng tốc độ chạy cho các ứng dụng phổ thông như trình duyệt, game) · 12.2 Hiện tượng vướng víu lượng tử & Cổng logic lượng tử: Trạng thái vướng víu entanglement, cổng Hadamard (H), cổng CNOT, các cổng Pauli X/Y/Z (why: Cơ sở cấu thành các thuật toán và mạch xử lý lượng tử; when: Khi thiết kế mạch logic lượng tử; pitfall: Hiện tượng mất kết hợp lượng tử decoherence do nhiễu môi trường khiến qubit bị mất trạng thái lượng tử) · 12.3 Thuật toán lượng tử & Ứng dụng: Giới thiệu nguyên lý Thuật toán Shor (bẻ khóa RSA) và Thuật toán Grover (tìm kiếm tăng tốc bình phương) (why: Chứng minh ưu thế tuyệt đối về mặt toán học của tính toán lượng tử; when: Khi phân tích rủi ro mật mã học hoặc tối ưu hóa hóa học/vật liệu; pitfall: Máy tính lượng tử hiện tại đang ở kỷ nguyên NISQ có tỷ lệ lỗi cao, chưa thể chạy các thuật toán này ở quy mô thương mại thực tế) · 12.4 Bài toán tính toán xác suất đầu ra mạch lượng tử: Bài tập tính toán sự biến đổi của vector trạng thái lượng tử sau khi đi qua cổng Hadamard và cổng CNOT, xác định xác suất đo được các trạng thái $|00\rangle, |01\rangle, |10\rangle, |11\rangle$ (why: Giúp làm quen với toán tử lượng tử và số học lượng tử cơ bản; when: Khi thiết kế các mạch lượng tử cơ bản; pitfall: Hành động đo đạc measurement sẽ làm sụp đổ trạng thái chồng chập lượng tử về trạng thái cổ điển) · 12.5 Thực hành: Trình giả lập mạch lượng tử đơn giản (Quantum Circuit Simulator) — kéo thả cổng H, CNOT để xây dựng mạch 2-qubit, quan sát trạng thái quả cầu Bloch và biểu đồ cột xác suất đo được. **quantum-sim.js build-out (module RIÊNG, KHÔNG mở rộng cpu-core.js):** vector trạng thái, cổng H/CNOT/Pauli, đo xác suất (self-test $|\alpha|^2+|\beta|^2=1$ + kết quả mạch Bell). Nhánh riêng, không tái dùng logic CPU cổ điển. _Tiên quyết: Bài 11._
