...
| 76 | Part 1, `docker/docker-compose.yml` | Thiếu cấu hình bắt buộc — container chết ngay | Theo yêu cầu tác giả, image đổi từ `postgres:17-alpine` sang `postgres:latest`. Đổi xong container chết ngay với `Error: in 18+, these Docker images are configured to store database data in a format which is compatible with "pg_ctlcluster" … there appears to be PostgreSQL data in: /var/lib/postgresql/data (unused mount/volume)`. Từ Postgres 18, image Docker lưu dữ liệu ở thư mục riêng theo major version, nên cách mount `forge-pgdata:/var/lib/postgresql/data` (có trong hầu hết hướng dẫn) không còn được dùng — và cụm "unused mount/volume" nghĩa là dữ liệu rơi vào lớp ghi của container, mất sạch khi container bị xoá. | **đã sửa** — thêm `PGDATA: /var/lib/postgresql/data/pgdata` vào compose (cả bài lẫn project rà soát) kèm callout giải thích. Đã chạy thật: `npm run db -- up -d --wait` cho cả 3 service đều `Healthy`, `\dt` trên `media_forge` vẫn đủ 6 bảng (không mất dữ liệu), `postgres --version` = 18.3. YAML trong bài lệch 0 dòng so với file đã chạy. GHI CHÚ cho lượt sau: `latest` là tag không ghim — máy sạch pull mới có thể ra PG19 và nâng major thì không đọc được data directory cũ. Tác giả đã được cảnh báo và vẫn chọn `latest` (17/08/2026), đừng mở lại. |
