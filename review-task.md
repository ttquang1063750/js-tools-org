...
## Đợt hai lăm — phát hiện từ người đọc thật, 17/08/2026

Người dùng làm theo Part 1 §7.4 và tắc. Nguyên nhân đã được xác nhận bằng chạy
thật, không phải suy đoán: người đọc đặt `synchronize` theo `NODE_ENV ===
'development'` trong `app.module.ts`, nên ở môi trường dev TypeORM tự tạo bảng
thẳng từ entity lúc app khởi động. Bằng chứng trên máy: database `media_forge`
có đủ 5 bảng nhưng KHÔNG có bảng `migrations`. Đến khi chạy `migration:generate`
thì entity ↔ DB đã khớp sẵn nên TypeORM báo `No changes in database schema were
found - cannot generate a migration`.

| # | Vị trí | Loại | Mô tả | Trạng thái |
|---|--------|------|-------|------------|
| 66 | Part 1, `src/database/typeorm.options.ts` (dòng `synchronize: false`) | Lời hứa không giao | Chú thích ghi "Bat buoc false. Xem canh bao ben duoi." nhưng KHÔNG có cảnh báo nào bên dưới — callout ngay sau đó nói về `typeorm-naming-strategies`. Trong cả Part 1, chữ `synchronize` xuất hiện đúng 1 lần là chính dòng code đó. Người đọc không bao giờ biết vì sao bắt buộc false, nên bật nó lên là chuyện tự nhiên. Cần viết đúng cảnh báo đã hứa: `synchronize: true` khiến TypeORM tự sửa schema theo entity mỗi lần khởi động — nó vô hiệu hoá toàn bộ hệ thống migration, và trên dữ liệu thật nó XOÁ cột/bảng không còn khớp entity mà không hỏi. | **đã sửa** — Part 1 §7.1 nay co callout --warning dung cho `synchronize`, neu ba he qua: vo hieu hoa migration (khong co bang `migrations`), `DROP COLUMN` khong hoi khi doi entity, va migration khong bao gio duoc chay thu tren DB dev. Da kiem: DB dev sau `migration:run` co dung 6 bang gom ca `migrations` |
| 67 | Part 1, §7.3 `app.module.ts` | Cạm bẫy cấu trúc | `useFactory` trải `...sharedOptions` rồi mới đặt các key phía sau, nên mọi key thêm vào sau đều GHI ĐÈ giá trị trong `sharedOptions` — kể cả `synchronize: false` đã đặt cẩn thận ở đó. Bài không nói gì về thứ tự này. Người đọc thêm một dòng theo mẫu `logging: ... === 'development'` rất dễ viết nhầm thành `synchronize: ... === 'development'` và tự vô hiệu hoá migration mà không nhận ra. Cần một câu ngay dưới khối code nói rõ: key đặt sau phần trải sẽ đè lên `sharedOptions`, nên đừng bao giờ đặt lại `synchronize` ở đây. | **đã sửa** — Part 1 §7.3 nay co callout --pitfall noi ro key dat sau `...sharedOptions` se de len no, neu dich danh cach viet nham `synchronize: ... === 'development'` theo mau cua `logging`, va quy tac: chi dat o day thu that su phu thuoc moi truong |
| 68 | Part 1, §7.4 "Sinh và chạy migration" | Lệnh không chạy được như viết | Bài chỉ in ra kết quả thành công của `migration:generate`, không nói gì về trường hợp database KHÔNG rỗng. Thông báo `No changes in database schema were found` không gợi ý bất cứ nguyên nhân nào, người đọc tắc hẳn. Cần thêm đoạn sau lệnh: generate là phép SO SÁNH entity ↔ DB thật; hai nguyên nhân cho thông báo đó là (a) DB đã có sẵn schema — thường do `synchronize: true` như ở #66/#67, (b) glob entity không khớp file nào; lệnh phân biệt là `docker exec forge-postgres psql -U forge -d media_forge -c '\dt'` (thấy bảng = (a), `No relations found` = (b)); cách làm sạch là `DROP SCHEMA public CASCADE; CREATE SCHEMA public;` rồi sinh lại. | **đã sửa** — Part 1 §7.4 nay co callout --pitfall + 2 khoi lenh. Nang muc do nghiem trong: ngoai thong bao "no changes", con truong hop DB co schema MOT PHAN thi lenh THANH CONG nhung sinh ra file ten InitSchema chi chua ALTER TABLE, khong co CREATE TABLE nao — da gap that trong project ra soat, no khi chay tren DB sach voi loi `relation "refresh_tokens" does not exist`. Bai nay them lenh chan doan `migration:show` va quy trinh don sach, kem dau hieu nhan biet file dung: phai co 5 lenh CREATE TABLE |
| 69 | Part 1, `src/database/data-source.ts` | Thiết kế yếu hơn mức bài tự đặt ra | File tự đọc `process.env['DATABASE_URL']` rồi `throw` viết tay, kèm chú thích "CLI không chạy qua ConfigModule nên phải tự kiểm". Đúng là ConfigService không dùng được ngoài Nest, nhưng kết luận "nên phải tự kiểm" là bước hụt: cách chuẩn là tách phần nạp + validate config ra một hàm Node thuần (`loadConfig()`) rồi CHO CẢ ConfigModule LẪN CLI cùng gọi. Hệ quả của cách hiện tại: (a) schema zod trong `configuration.ts` không chạy trên đường CLI, nên một `DATABASE_URL` sai định dạng mà app từ chối thì CLI vẫn nhận; (b) `config()` gọi trần phụ thuộc cwd — chạy lệnh migration từ thư mục khác là không thấy `.env` mà không báo gì; (c) hai nơi cùng biết "cần DATABASE_URL" nên có thể lệch. Mẫu tham chiếu người dùng đưa ra: `/Users/qtang/Projects/fullstack/apps/api-nest/src/config/load-config.ts` (zod + cache + `findEnvFile()` đi ngược cây thư mục), được gọi ở cả `data-source.ts:25` và `config.module.ts:27`. Series bán điểm mạnh là giải thích "vì sao", nên đây đúng chỗ cần nói rõ đánh đổi thay vì chọn cách yếu hơn mà không nhắc tới. | **đã sửa** — Part 1 §7.1 nay đưa cách hay gặp ra làm phản ví dụ, giải thích vì sao nó tạo nguồn sự thật thứ hai, rồi dùng `validateEnv(process.env)` với `.env` giải theo `resolve(__dirname, ...)`. Đã chạy thật trong `~/Projects/Scratchpad/media-forge`: `migration:show` chạy bình thường; `DATABASE_URL=mysql://...` bị chặn đúng thông báo zod `must start with "postgres://"` ném từ `validateEnv`; nạp được DataSource với cwd=/private/tmp (dotenv vẫn thấy đúng `.env`); `npx tsc --noEmit` sạch. Phát sinh thêm một cạm bẫy đã ghi vào bài: thêm `export default` bên cạnh export có tên làm CLI chết với `must contain only one export of DataSource instance` |

| 70 | Part 1, §8.2 "Tái hiện lỗi" | Sai thứ tự — lệnh không thể chạy được | Bài bảo người đọc bắn `POST http://localhost:3000/billing/charge` (vị trí ký tự 92613), nhưng route đó chưa tồn tại: `@Controller('billing')` xuất hiện lần đầu ở 103601, `billing.module.ts` ở 104192, và dòng đăng ký `BillingModule` trong `app.module.ts` ở 105209 — tức là SAU lệnh gọi khoảng 11.000 ký tự, tận sau §8.3. `charge.dto.ts` (100804) mà controller import cũng nằm sau. Người đọc làm đúng thứ tự chắc chắn nhận 404 mười lần, không có cách nào khác. Người dùng đã gặp đúng như vậy. Cách sửa: chuyển 4 khối `charge.dto.ts` + `billing.controller.ts` + `billing.module.ts` + `app.module.ts (thêm BillingModule)` lên TRƯỚC §8.2, và nói rõ ở đó rằng service vẫn giữ bản sai của §8.1 — nếu không người đọc sẽ lấy luôn bản vá ở §8.3 và không tái hiện được lỗi. Lưu ý: các lượt rà soát trước không bắt được vì project đã được dựng sẵn đủ file, không đi đúng trình tự bài. | **đã sửa trong bài, CHƯA chạy xác nhận** — đã chuyển cả 4 khối (`charge.dto.ts`, `main.ts` ValidationPipe, `billing.controller.ts`, `billing.module.ts`, `app.module.ts +BillingModule`) lên trước lệnh curl của §8.2, thêm callout "giữ nguyên billing.service.ts bản sai ở trên", và sửa câu mở đầu §8.3 cho liền mạch. Thứ tự mới đã kiểm bằng vị trí ký tự: controller 95395 < đăng ký module 98046 < curl 99176 < §8.3 103942. CHƯA chạy được để xác nhận 10×201: project rà soát đã tiến tới Part 3/4 (`job.runner.ts` cần `chargeForJob`) nên không hạ service về bản §8.1 được, và môi trường Postgres đang hỏng (volume chứa dữ liệu PG18 trong khi compose của bài dùng postgres:17-alpine — container cũ do một phiên trước dựng ngoài compose). Đã bổ sung thêm (gợi ý của người đọc): một smoke test gọi 1 request khi số dư = 0, kỳ vọng `400 Không đủ credit`, kèm bảng đọc mã trả về (404 = chưa cắm module/chưa restart, 500 = chưa migration:run, 201 = số dư sót từ lần trước). Nó tách "sai wiring" khỏi "race condition" trước khi vào bài kiểm ACID. Lượt sau: dựng một project Part-1-only từ THƯ MỤC TRỐNG, gõ theo đúng thứ tự bài, rồi chạy §8.2 — kỳ vọng 10 lần `201` và số dư âm. |

### Trạng thái project rà soát sau đợt này (17/08/2026)

`~/Projects/Scratchpad/media-forge` đã được dựng lại phần migration, vì file
`InitSchema` cũ trong đó bị sinh trên database không rỗng nên nội dung sai (chỉ
có `ALTER TABLE`, không có `CREATE TABLE` nào) — chính là ca #68 mô tả.

Đã làm: `DROP SCHEMA public CASCADE` → xoá cả hai file migration cũ →
`migration:generate InitSchema` trên DB rỗng (ra đủ 5 `CREATE TABLE`) →
`migration:run` (DB có 6 bảng, gồm `migrations`).

**Lưu ý cho lượt rà soát sau:** lịch sử migration của project đã bị gộp lại làm
một. File `AddReplacedByHash` của Part 2 không còn, vì cột `replaced_by_hash`
nay nằm sẵn trong `InitSchema`. Muốn kiểm lại đúng bước Part 2 §… thì phải dựng
lại project từ đầu với bộ entity của riêng Part 1.

| 71 | Part 2, §3.3 "Xoay vòng, và cái bẫy khi hai tab cùng refresh" (@50141), khối Terminal @60358 | Sai thứ tự — lệnh không thể chạy được | Cùng loại với #70, ở Part 2. Khối "Terminal — tự thấy cơ chế thu hồi family" bảo người đọc chạy `curl -X POST localhost:3000/auth/login` rồi `/auth/refresh` (4 lời gọi), nhưng `AuthController` với `@Post('login')`/`@Post('refresh')` chỉ xuất hiện ở §3.5 "Trả refresh token qua cookie hay qua body" @67395 — sau đó ~7.000 ký tự. Người đọc làm đúng thứ tự nhận 404. Nghiêm trọng vì đây là thứ đầu tiên người đọc thử ở Part 2, và mọi phần sau đều cần token. Cách sửa: như #70 — chuyển `auth.controller.ts` (+ DTO của nó, + đăng ký `AuthModule`) lên trước khối Terminal của §3.3, hoặc dời khối demo đó xuống sau §3.5. | **đã sửa** — Đã chuyển khối demo "tự thấy cơ chế thu hồi family" (cùng đoạn giải thích bước 3) xuống SAU khối `auth.controller.ts` của §3.5, và để lại ở §3.3 một con trỏ tiến nói rõ vì sao phải chờ. Thứ tự mới: `auth.module.ts` 22604 < `app.module.ts +AuthModule` 24549 < `auth.controller.ts` 67571 < demo curl 71171. `scan-call-order.py`: 0 vi phạm trên cả 4 part (trước đó 8) |
| 72 | Part 2, toàn bài | Bước bắt buộc không bao giờ được nói | Không chỗ nào trong Part 2 dặn thêm `AuthModule` vào `imports` của `AppModule`. Chỗ duy nhất nhắc tới là một câu HỒI TỐ ở @98991: "thêm RedisModule vào mảng imports của AppModule, **đúng chỗ đã thêm AuthModule ở mục 2**" — nhưng mục 2 chưa từng dặn. Đã kiểm: `class AppModule` xuất hiện 0 lần trong part-2/3/4, và mọi `imports: [...]` của Part 2 chỉ là của module cấp tính năng. Hệ quả: dù đã sửa #71 thì `/auth/login` vẫn 404, vì Nest không khởi tạo `AuthController`. Đối chiếu: `MediaModule` (@117364), `JobModule` (Part 3 @49490), `GatewayModule` (Part 3 @96435), `RedisModule` (@98936), `BlockController` (Part 3 @10750) đều CÓ câu dặn bằng văn xuôi đặt trước bài kiểm tương ứng — chỉ `AuthModule` bị sót. | **đã sửa** — Đã thêm ngay sau `auth.module.ts` ở §2: một đoạn giải thích + khối `src/app.module.ts — thêm AuthModule`, kèm callout nói rõ từ đó trở đi bài chỉ viết một câu "thêm XModule vào imports của AppModule" thay vì lặp lại cả khối. Nhờ vậy câu hồi tố ở @98991 ("đúng chỗ đã thêm AuthModule ở mục 2") giờ đúng sự thật |

| 73 | Part 1, §8.4 "Còn cách nào khác" | Quyết định thiết kế nêu mà không có code | Người đọc hỏi thẳng: "có code mẫu luôn không?" — không có. §8.4 chỉ có một khối ASCII so sánh ba cách; hai trong ba cách (SERIALIZABLE, khoá lạc quan) chỉ được kể lại. Riêng câu "khó áp cho mô hình sổ cái chỉ INSERT" thì người đọc không có cách nào tự kiểm. Trong một loạt bài lấy điểm mạnh là chỉ ra "vì sao" bằng code thì đây là lỗ rõ. | **đã sửa** — thêm §8.4 hai mục con có code đầy đủ + kết quả chạy thật: (1) `chargeSerializable` với vòng retry chỉ bắt SQLSTATE 40001, đã chạy 10 request song song trên DB `media_forge_lab`, ra 5×201 (`attempts` 1,1,1,2,2; lần chạy khác 1,1,2,2,3) + 5×400, số dư về 0 — và nói rõ chi phí thật: transaction chạy gần xong vẫn bị huỷ nên mọi việc trong nó phải chịu được chạy hai lần; (2) `optimistic-demo.mjs` dùng cột version, chạy ra 5 thành công, `version = 5`, balance 0 — và chính dòng `CREATE TABLE demo_optimistic` là bằng chứng cho câu "khó áp cho sổ cái chỉ INSERT": phải có một dòng để đặt version lên, tức phải bỏ sổ cái append-only và mất khả năng trả lời "50 credit đó đến từ đâu". Đã đối chiếu tự động: code trong bài lệch 0 dòng so với code đã chạy. |

| 74 | Part 1, `docker/docker-compose.yml` | Cấu hình viết cứng, và một cạm bẫy thất bại im lặng | Yêu cầu của tác giả: compose phải đọc `.env` thay vì viết cứng `POSTGRES_USER/PASSWORD/DB`. | **đã sửa** — compose nay dùng `${POSTGRES_USER}` / `${POSTGRES_PASSWORD}` / `${POSTGRES_DB}` (cả trong `healthcheck`), `.env.example` thêm ba biến kèm ghi chú "chỉ dành cho compose, không nằm trong schema zod, đổi POSTGRES_DB thì phải đổi cả DATABASE_URL". Kèm callout về cạm bẫy đã ĐO ĐƯỢC: compose lấy thư mục chứa file YAML làm project directory, nên `docker compose -f docker/docker-compose.yml up -d` chạy từ gốc dự án KHÔNG đọc `.env` ở gốc — nó chỉ in `level=warning ... variable is not set. Defaulting to a blank string.` rồi tạo container với user/mật khẩu rỗng. Cách chắc chắn là `--env-file .env`, và để không phải nhớ cờ đó thì gói vào script `"db": "docker compose --env-file .env -f docker/docker-compose.yml"`; mọi lệnh compose ở Part 1 và Part 2 đã đổi sang `npm run db -- ...`. Đã chạy thật: không có cờ → 5 cảnh báo "not set"; qua `npm run db` → 0 cảnh báo, `config` in ra đúng `POSTGRES_DB: media_forge`, healthcheck giải thành `pg_isready -U forge -d media_forge`. Đối chiếu tự động: YAML trong bài lệch 0 dòng so với file đã chạy. |

| 75 | Part 1, khối Terminal sau `docker-compose.yml` | Chỉ dẫn không thể làm theo | Người đọc hỏi thẳng: bài viết "Đợi tới khi healthy rồi hãy chạy app" nhưng chỉ đưa `npm run db -- ps` — mà `ps` chụp một lần, không đợi gì cả. Không có cách nào để đợi, người đọc phải tự gõ lại nhiều lần và tự đoán khi nào thì đủ. | **đã sửa** — đổi sang `npm run db -- up -d --wait`, kèm output thật (`Waiting` → `Healthy`) và giải thích: không có cờ đó thì `up -d` trả về ngay khi container được TẠO, còn Postgres bên trong vẫn đang khởi động. `--wait` đọc chính `healthcheck` đã khai ở trên nên không cần vòng lặp; thêm `--wait-timeout 60` để không treo mãi. Đã chạy thật: `npm run db -- up -d --wait redis` in `Waiting` rồi `Healthy` và chỉ trả về sau đó. Ghi rõ cờ này có từ Compose v2.1.1 (máy kiểm: v5.3.1) và đưa kèm vòng lặp `until docker exec ... pg_isready` cho máy cũ hơn — vòng lặp này cũng đã chạy thật. |

| 76 | Part 1, `docker/docker-compose.yml` | Thiếu cấu hình bắt buộc — container chết ngay | Đổi image khỏi `postgres:17-alpine` làm container chết ngay với `Error: in 18+, these Docker images are configured to store database data in a format which is compatible with "pg_ctlcluster" … there appears to be PostgreSQL data in: /var/lib/postgresql/data (unused mount/volume)`. Từ Postgres 18, image Docker lưu dữ liệu ở thư mục riêng theo major version, nên cách mount `forge-pgdata:/var/lib/postgresql/data` (có trong hầu hết hướng dẫn) không còn được dùng — cụm "unused mount/volume" nghĩa là dữ liệu rơi vào lớp ghi của container, mất sạch khi container bị xoá. | **đã sửa** — thêm `PGDATA: /var/lib/postgresql/data/pgdata`, và image ghim thành **`postgres:18`** (tác giả ban đầu chọn `latest`, sau đó đổi sang ghim theo khuyến nghị 17/08/2026). Chọn `18` chứ không `18-alpine` vì cluster đã `initdb` dưới image Debian với locale `en_US.utf8` — đổi sang alpine là đổi sang musl libc, tức đổi nền collation dưới một data directory đã tồn tại. Callout trong bài giải thích cả hai điều: vì sao cần `PGDATA`, và vì sao phải ghim major (latest sẽ theo lên PG19, nâng major thì không đọc được data directory cũ). Đã chạy thật: `npm run db -- up -d --wait` cho cả 3 service `Healthy`, `postgres --version` = 18.6 (ghim major vẫn nhận bản vá minor), `\dt` trên `media_forge` đủ 6 bảng, `datcollate` vẫn `en_US.utf8`. YAML trong bài lệch 0 dòng so với file đã chạy. |
...
| 81 | Part 2, §2.1 `src/billing/billing.controller.ts` | Bước bắt buộc bị bỏ sót | Người đọc báo: `Nest can't resolve dependencies of the JwtAuthGuard (?). Please make sure that the argument JwtService at index [0] is available in the BillingModule module`. Bài gắn `@UseGuards(JwtAuthGuard)` lên `BillingController` (@34518) nhưng KHÔNG có khối `billing.module.ts` nào trong cả Part 2 — module không bao giờ được cập nhật để import `AuthModule`. Trớ trêu là bài CÓ dạy đúng quy tắc này, nhưng ở @122044 cho `MediaModule` — sau chỗ nó cắn 87.500 ký tự. | **đã sửa** — thêm ngay sau khối controller: callout in đúng nguyên văn thông báo lỗi, giải thích vì sao lỗi chỉ vào `BillingModule` dù guard nằm ở `auth/` (decorator nằm trên controller của BillingModule nên Nest tìm `JwtService` trong ngữ cảnh đó), nêu quy tắc chung "module nào gắn guard thì module đó phải import module cung cấp phụ thuộc của guard", và khối `billing.module.ts — thêm AuthModule`. Đã quét cả 4 part: 5 controller có `@UseGuards`, và sau bản vá thì mọi module chứa chúng đều import `AuthModule` — chỉ `BillingModule` từng thiếu. |

| 82 | Part 2, §3.2 `src/auth/auth.service.ts — phát token` | Code không có đoạn chốt, đúng chỗ quan trọng nhất | Người đọc báo: "familyId được gán tới nhiều call parameter nhưng không thấy nó được set ở đâu". Code thì ĐÚNG — `login()` gọi `this.issuePair(user, randomUUID())` — nhưng dòng đó nằm cuối một khối ~40 dòng, và văn xuôi hai bên khối nói về SHA-256 vs argon2 (trước) và `Math.random()` (sau), không chỗ nào nhắc `familyId` sinh ra ở đâu. Chỗ duy nhất đỡ nó là một comment trong code. Trong khi family lại là khái niệm trung tâm của §3.3 (thu hồi cả family), nên người đọc cần mô hình này TRƯỚC khi vào mục đó. | **đã sửa** — thêm đoạn chốt sau khối, chỉ đích danh `this.issuePair(user, randomUUID())` là chỗ DUY NHẤT trong cả dự án sinh `familyId` mới, kèm ba quy tắc vòng đời: đăng nhập mở family mới (mỗi thiết bị một chuỗi); xoay vòng giữ nguyên `row.familyId`; phát hiện dùng lại thì `delete({ familyId })` quét sạch cả chuỗi. |

| 83 | Part 2, §3.1 + §3.3 `replaced_by_hash` | Thuật ngữ dùng mà không định nghĩa + văn xuôi mô tả SAI việc code làm | Người đọc: "đọc mãi mà không hiểu replacedByHash dùng để làm gì". Đúng — cả bài chỉ có một comment trong entity ("Token nào đã thay thế token này. Dùng cho khoảng ân hạn ở mục 3.3") và một câu "nó phục vụ khoảng ân hạn ở mục 3.3", rồi §3.3 dùng luôn `row.replacedByHash` mà không nói nó lưu gì hay vì sao cần. Nặng hơn: cả văn xuôi ("trả lại đúng cặp token đã phát ở lần đó") lẫn comment trong code ("Tra lai dung token da phat cho tab thu nhat") đều SAI so với code — `reissueFrom` trên một row chưa dùng thì phát cặp MỚI và đánh dấu row đó đã dùng, nên nó xoay tiếp một nhịp chứ không trả lại token tab 1 đang giữ. | **đã sửa** — thêm 4 đoạn trước khối ân hạn: (1) `replaced_by_hash` lưu hash của token vừa thay thế, ghi ở dòng cuối `reissueFrom`; (2) vì sao cần con trỏ đó — token cũ không có cách nào biết mắt xích còn sống của chuỗi trừ khi lúc xoay vòng đã ghi lại, và lưu hash chứ không lưu token gốc cùng lý do với `token_hash`; (3) nói chính xác điều xảy ra: nó xoay tiếp và trả token MỚI, cả hai tab đều cầm token hợp lệ cùng family, giá phải trả là tiêu thêm một mắt xích; (4) ranh giới: ân hạn chỉ che trong `GRACE_MS`, tab ngủ quên mười phút vẫn bị thu hồi family — đúng thiết kế. Đã sửa cả câu văn lẫn comment cho khớp code. |

| 84 | Part 2, §3.3 chuyển từ `reissueFrom` sang `tryReissue` | Đổi tên không báo trước | Người đọc: "bạn giới thiệu reissueFrom nhưng cuối cùng code lại là tryReissue? giới thiệu để làm gì, nó càng phức tạp hoá vấn đề". Bài CÓ giải thích, nhưng đặt SAU khối code cuối, và mô tả bản đầu là "chỉ để dễ theo dõi từng mảnh riêng lẻ" — bán rẻ lý do thật. Người đọc gặp cái tên mới mà không có tín hiệu nào báo trước, cũng không biết vì sao phải học một hàm rồi bỏ. | **đã sửa** — thêm callout NGAY TRƯỚC khối bản cuối: nói rõ vẫn là hàm đó, tên đổi vì HỢP ĐỒNG đổi (bản cũ `throw` tại chỗ, bản mới trả `{ kind: 'ok' \| 'reused' }` để `refresh()` quyết sau khi transaction đóng — tiền tố `try` là quy ước cho đúng chuyện đó), thừa nhận thẳng sự khó chịu, và nêu lý do đáng đổi: bản đầu không sai cú pháp cũng không sai logic khi đọc từng dòng, nó chỉ âm thầm không đốt được family vì rollback — đọc thẳng bản đúng thì không bao giờ biết cái bẫy đó tồn tại. Đã sửa câu sau khối cho khớp lý do thật. **Không** bỏ bản 1: cái bẫy rollback là bài học chính của mục, và đây là khuôn quen thuộc của series (Part 1 §8.1 sai → §8.3 vá). |

### Bài học về quy trình rà soát (17/08/2026)

#70 không phải do đọc sót — nó vô hình theo đúng nghĩa cấu trúc. Cả các lượt rà
soát trước đều dựng project với ĐỦ FILE trước rồi mới chạy `curl`. Trong một cây
code đã có `billing.controller.ts` sẵn thì lệnh curl ở §8.2 trả `201`, nên lỗi
thứ tự không thể lộ ra bằng cách kiểm đó.

Nói cho đúng: "đã chạy và được" ở các lượt trước chỉ chứng minh **endpoint hoạt
động khi đã cắm dây**, không chứng minh **trình tự của bài chạy được**. Hai khẳng
định khác nhau.
